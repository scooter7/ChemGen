  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const result = await signIn('credentials', {
        ...
        email,
        password,
        // callbackUrl: callbackUrl // next-auth handles callbackUrl on successful redirect from its side if redirect is true
      });
      ...
      if (result?.error) {
        ...
        // Successful sign-in
        // result.url will be null if redirect: false, so we use our callbackUrl
        router.push(callbackUrl); // Manually redirect to the intended page or dashboard
      } else {
        // This case should ideally not be reached if result.error or result.ok is always set
