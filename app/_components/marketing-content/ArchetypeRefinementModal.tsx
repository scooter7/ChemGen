...
export default function ArchetypeRefinementModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-opacity duration-300 ease-in-out">
      <div className="bg-gray-800 p-6 rounded-lg shadow-2xl w-full max-w-md space-y-6 transform transition-all duration-300 ease-in-out scale-100 opacity-100">
        <div className="flex justify-between items-center">
          <h3 className="text-xl font-semibold text-white">Archetype Refinement</h3>
          <button 
            onClick={onClose} 
            className="p-1.5 rounded-full text-gray-400 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            aria-label="Close"
          >
            <X size={22} />
          </button>
            ...
            <div key={archetype.name} className="space-y-2 p-1">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <span 
                    className="w-4 h-4 rounded-full mr-2.5 flex-shrink-0 border border-gray-600" 
                    style={{ backgroundColor: archetype.color || '#E0E0E0' }}
                    title={archetype.name}
                  ></span>
                  <label htmlFor={`text-${archetype.name.replace(/\s+/g, '-')}`} className="text-sm font-medium text-gray-200">
                    {archetype.name}
                  </label>
                </div>
                <input
                  ...
                  className="w-20 text-sm font-semibold p-1.5 border border-gray-600 rounded-md text-center bg-gray-700 text-white focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                />
              </div>
              <input
        ...
        <div className={`text-sm font-medium text-center py-2 px-3 rounded-md transition-colors ${totalPercentage === 100 ? 'bg-green-800/40 text-green-300' : 'bg-red-800/40 text-red-300'}`}>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center pt-4 border-t border-gray-700 space-y-3 sm:space-y-0">
           <button
            ...
            className="w-full sm:w-auto order-last sm:order-first px-4 py-2 text-sm border border-gray-500 rounded-md text-gray-300 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800"
          >
            Distribute Evenly
          </button>
          <div className="flex space-x-3 w-full sm:w-auto">
            <button
              onClick={onClose}
              className="w-1/2 sm:w-auto flex-grow sm:flex-grow-0 px-4 py-2 text-sm border border-gray-500 rounded-md text-gray-300 hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800"
            >
              Cancel
            </button>
            <button
              ...
              className="w-1/2 sm:w-auto flex-grow sm:flex-grow-0 px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-md shadow-sm disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800"
            >
              Apply
            </button>
          </div>
