      ...
      <div className="bg-gray-800 shadow-xl rounded-lg p-6 md:p-8">
        ...
        {isLoading && (
          <div className="flex justify-center items-center py-10">
            ...
            <p className="ml-3 text-lg text-gray-300">Loading history...</p>
          </div>
        )}

        {error && (
          <div className="p-4 bg-red-900/30 text-red-300 rounded-md text-sm flex items-center">
        ...
        {!isLoading && !error && history.length === 0 && (
          <div className="text-center py-10">
            <p className="text-gray-400">You have no saved content history yet.</p>
          </div>
        )}
        ...
        {!isLoading && !error && history.length > 0 && (
          <div className="space-y-6">
            {history.map((item) => (
              <div key={item.id} className="border border-gray-700 rounded-lg p-4 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex-grow">
                    <p className="text-sm font-semibold text-gray-100">Prompt:</p>
                    <p className="text-sm text-gray-300 italic">&quot;{item.promptText}&quot;</p>
                ...
                {item.justification && (
                    <details className="mt-3 text-xs">
                        <summary className="cursor-pointer font-medium text-indigo-400">View Justification</summary>
                        <p className="mt-1 p-2 bg-slate-700 rounded text-gray-300">{item.justification}</p>
                    </details>
                )}
                <div className="prose prose-sm prose-invert max-w-none mt-4 p-3 border-t border-gray-600">
